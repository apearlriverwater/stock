...
g_look_back_weeks=max(40,g_week_in_trade_day*3)  #回溯分析的周期数
...
def create_market_data(stock,start_DateTime,stop_DateTime,
    ...
    g_market_train_data=gmTools.read_kline(stock,int(week*60),
            start_DateTime,stop_DateTime,30000)    #训练数据
...
def setup_tensor(sess,stock,week,last_utc,next_train_time=0):
    ...
    with tf.name_scope('accuracy'):
        with tf.name_scope('buy_prediction'):
            buy_prediction = tf.greater_equal(tf.argmax(y1, 1),5)
...
def main(stock=g_test_securities, week=g_week,look_back_weeks=g_look_back_weeks):
    ...
    for stock in g_test_securities:
        ...
        next_train_utc=  g_max_step-g_look_back_weeks*2-1

        if next_train_utc<=0 and model_trained==False or g_max_step==0:
            i=0
            test_writer.close()
            train_writer.close()
        ...
        elif next_train_utc<=0 and model_trained==True:
            #后续少量数据分析，下一次分析仅针对新数据
            next_train_utc=g_max_step-1

        #当前数据的终止时间与上次分析的终止时间相等，直接退出
        if last_utc==g_market_train_data['endtime'][g_max_step - 1]:
            print("no new data,pass analyse !")
            test_writer.close()
            train_writer.close()
            g_market_train_data = 0
            g_normalized_data = 0
        ...
        if model_trained==False:
            begin_buy_sell_analyse=int(g_max_step*0.3)
        else:
            begin_buy_sell_analyse=0

        i=look_back_weeks
        output_count = i
        print("analysing %s" % (gmTools.timestamp_datetime(
            g_market_train_data['endtime'][i])))
        ...
        for i in range(look_back_weeks,g_max_step):
            if True:
                if i % 100 == 0:
                    ...
                    #
                    summary,_,is_sell,is_buy= sess.run([merged,train_step,
                        sell_prediction, buy_prediction],feed_dict=feed_dict_data)

                    train_writer.add_summary(summary, i)

                    #训练一段时间后才进行买卖点分析
                    if model_trained or i>begin_buy_sell_analyse:
            ...
            if i - output_count > int(g_max_step/6):
                # 防止长时间计算无输出
                output_count = i
                print("analysing %s"%(gmTools.timestamp_datetime(
                    g_market_train_data['endtime'][i])))
        ...
        if buy_count>0:
            ...
            print ("total buy:%d,sucess:%.2f%%,buy_ok =%d"%(
                buy_count,buy_ok*100/buy_count,buy_ok))

        else:
            print("no trade ")
        if i>0:
            print("end in [%s]\n" % (gmTools.timestamp_datetime(
                g_market_train_data['endtime'][i])))
        else:
            print("end no datas\n")
        '''
        ...
        if i > 0 and next_train_utc<g_max_step-1:
